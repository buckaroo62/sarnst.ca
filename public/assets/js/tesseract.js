...
class Tesseract {
   ...
   // from the bits while we count
	...
	doTesseract () {
      ...
      // I don't even know what to call them al
	...
	// Build a simple rotation matrix for XY
	getRotXY(ang) {
		// The angle has to be in radians
        // What are radians?
      // They are the SI (International Standard) for measuring angles
      // As opposed to "degrees".
      // One radian is the angle of one radius around a circle
      // PI radians is half way around a circle (or 180 degrees)
      // Don't be scared of radians. Feel the zen
      // https://www.youtube.com/watch?v=HACNCy0clO0
      // And the cosine is not a magical number:
      // It is the x coordinate of a unit circle
      // that we have travelled around theta radians (or degrees)
	...
	draw4Dlines (p) {
		// Using this, we can draw all of the edges from one to the next without repeating and without drawing over any one twice
		// (we can't do that with a cube)
		let disp = [0, 1, 3, 2, 6, 14, 10, 8, 9, 11, 3, 7, 15, 14, 12, 13, 9, 1, 5, 7, 6, 4, 12, 8, 0, 4, 5, 13, 15, 11, 10, 2, 0];
